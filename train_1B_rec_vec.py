...
def main():
    ...
    transformer3d, optimizer, train_rec_dataloader, lr_scheduler = accelerator.prepare(transformer3d, optimizer, train_rec_dataloader, lr_scheduler)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_rec_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                # print("----------------------------------------------")
                # print("check dimensions of all the inputs")
                # print(f"vocal_seq_len: {vocal_seq_len}")  # [81]
                # print(f"pixel_values: {pixel_values.size()}")  # [1, 81, 3, 512, 512]
                # print(f"masked_pixel_values: {masked_pixel_values.size()}")  # [1, 81, 3, 512, 512]
                # print(f"pixel_value_masks: {pixel_value_masks.size()}")  # [1, 4, 21, 64, 64]
                # print(f"reference_image: {reference_image.size()}")  # [1, 3, 512, 512]
                # print(f"vocal_input_values: {vocal_input_values.size()}") # [1, 43200]
                # print(f"The size of tgt_face_masks: {tgt_face_masks.size()}")  # [1, 81, 3, 512, 512]
                # print("----------------------------------------------")
                ...
                with torch.no_grad():
                    ...
                    # print(f"The size of tgt_face_masks: {tgt_face_masks.size()}")  # [1, 16, 21, 64, 64]
                    # print(f"The size of latents: {latents.size()}")  # [1, 16, 21, 64, 64]
                    # print(f"The size of pixel_value_masks: {pixel_value_masks.size()}") # [1, 4, 21, 64, 64]

                    # tgt_face_masks = _batch_encode_vae(tgt_face_masks)
                    # tgt_face_masks = tgt_face_masks.clamp(0, 1)
                    # tgt_lip_masks = _batch_encode_vae(tgt_lip_masks)
                    # tgt_lip_masks = tgt_lip_masks.clamp(0, 1)
                    ...
                    # print(f"The size of masked_latents: {masked_latents.size()}")  # [1, 16, 21, 64, 64]
                    # print(f"The size of pixel_value_masks: {pixel_value_masks.size()}")  # [1, 4, 21, 64, 64]
                    # print(f"The size of clip_pixel_value: {clip_pixel_values.size()}")  # [1, 3, 512, 512]
                    # print(f"The size of clip_context: {clip_context.size()}")  # [1, 257, 1280]
                ...
                with torch.no_grad():
                    ...
                    # print(f"The size of vocal_embeddings: {vocal_embeddings.size()}") # [1, 134, 768] or [1, 168, 768]
                ...
                # print(f"seq_len: {seq_len}")  # 21504
                # print(f"The size of clip_context: {clip_context.size()}")  # [1, 257, 1280]

                # Predict the noise residual
                with torch.cuda.amp.autocast(dtype=weight_dtype):

                    # print(motion_type) # speech
                    # print(type(motion_type)) # <class 'str'>
                    # print(type(batch['text_prompt'])) # <class 'list'>
                    # print(batch['text_prompt']) # ['']

                    # print("check whether you want to utilize audio_strength_features")

                    # print(f"vocal_context_lens: {vocal_context_lens}")  #  [ 5, 11, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 12]
                    # print(f"The size of vocal_proj_feature: {vocal_proj_feature.size()}") # [1, 134, 2048]
                    # print(f"The size of vocal_proj_split: {vocal_proj_split.size()}") # [1, 21, 15, 2048]
                ...
                def custom_mse_loss(noise_pred, target, weighting=None, threshold=50, tgt_face_masks=None, tgt_lip_masks=None):
                    ...
                    # training setting 2.0
                    # mask_loss_flag = torch.rand(1).item()
                    # if mask_loss_flag <= 0.2:
                    #     mse_loss = mse_loss * (1 + tgt_face_masks + tgt_lip_masks)
                    # else:
                    #     mse_loss = mse_loss * tgt_lip_masks

                    # training setting 3.0
                    # mask_loss_flag = torch.rand(1).item()
                    # if mask_loss_flag <= 0.2:
                    #     mse_loss = mse_loss
                    # elif mask_loss_flag > 0.2 and mask_loss_flag < 0.4:
                    #     mse_loss = mse_loss * (tgt_face_masks + tgt_lip_masks)
                    # else:
                    #     mse_loss = mse_loss * tgt_lip_masks
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if args.use_deepspeed or accelerator.is_main_process:
                        ...
                        # vocal_projector_saved_path = os.path.join(args.output_dir, f"checkpoint-{global_step}", f"vocal-checkpoint-{global_step}.pt")
                        # unwrap_vocal = accelerator.unwrap_model(vocal_projector)
                        # unwrap_vocal_state_dict = unwrap_vocal.state_dict()
                        # torch.save(unwrap_vocal_state_dict, vocal_projector_saved_path)
                        # print(f"Saved vocal_projector to {vocal_projector_saved_path}")
                        #
                        # audio_projector_saved_path = os.path.join(args.output_dir, f"checkpoint-{global_step}", f"audio-checkpoint-{global_step}.pt")
                        # unwrap_audio = accelerator.unwrap_model(audio_projector_model)
                        # unwrap_audio_state_dict = unwrap_audio.state_dict()
                        # torch.save(unwrap_audio_state_dict, audio_projector_saved_path)
                        # print(f"Saved audio_projector to {audio_projector_saved_path}")
                        #
                        # motion_bucket_saved_path = os.path.join(args.output_dir, f"checkpoint-{global_step}", f"motion-checkpoint-{global_step}.pt")
                        # unwrap_motion = accelerator.unwrap_model(motion_bucket_model)
                        # unwrap_motion_state_dict = unwrap_motion.state_dict()
                        # torch.save(unwrap_motion_state_dict, motion_bucket_saved_path)
                        # print(f"Saved motion_bucket_model to {motion_bucket_saved_path}")
